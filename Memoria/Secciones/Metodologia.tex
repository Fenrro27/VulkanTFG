...
La simulación de partículas plantea un desafío computacional crítico: la ejecución fluida de millones de operaciones matemáticas por segundo.

El \textbf{enfoque clásico en CPU} trabaja de forma secuencial. El procesador calcula el estado de cada partícula. 
Una vez obtenidos todos los resultados, se transfiere todo ese bloque de datos hacia la memoria de vídeo (VRAM) usando el bus PCI-Express. 
Esta transferencia continua satura el canal de comunicación. Se genera un cuello de botella crítico. 
...
Por el contrario, el enfoque basado en GPU (GPGPU) ejecuta las actualizaciones de forma paralela, reuniendo y preservando la información en la memoria gráfica 
y ejecutando la física en los \textit{Compute Shaders}~\parencite{akenine2018real}.

Como los datos ya no viajan por el bus PCI-Express, el rendimiento mejora notablemente. La tasa de fotogramas se ve incrementada de forma drástica.
...
Su fuerte es el cálculo matemático bruto. En lugar de procesar tareas pesadas en cadena, la tarjeta gráfica despliega miles de hilos ligeros de forma simultánea. 
