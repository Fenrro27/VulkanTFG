 ...
 * @brief Archivo GEDescriptorSet.cpp
 */
...
//
// FUNCIï¿½N:GEDescriptorSet::GEDescriptorSet(GEGraphicsContext* gc, std::vector<GEUniformBuffer> ubos)
// 
// PROPï¿½SITO: Crea los conjuntos de descriptores asociados a los buffers
//
GEDescriptorSet::GEDescriptorSet(GEGraphicsContext* gc, GERenderingContext* rc, std::vector<GEUniformBuffer*> ubos, std::vector<GETexture*> tex)
{
	...
	for (size_t i = 0; i < imageCount; i++)
	{
		...
		if (textureCount > 0) {
			...
			wtex.dstBinding = bufferCount; // La textura va al binding despuÃ©s de los UBOs
...
//
// FUNCIï¿½N: GEDescriptorSet::destroy(GEGraphicsContext* gc) 
//
// PROPï¿½SITO: Destruye los conjuntos de descriptores
//
/**
 * @brief FunciÃ³n GEDescriptorSet::destroy
 */
void GEDescriptorSet::destroy(GEGraphicsContext* gc)
{
	// Esta lÃ­nea es redundante si se va a destruir el pool justo despuÃ©s
